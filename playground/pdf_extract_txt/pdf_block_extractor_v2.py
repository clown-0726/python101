...
class PdfBlockExtract:
    ...
    def loop_pages(self, page_layout):
        ...
        for element in page_layout:
            ...
            if isinstance(element, LTTextContainer):
                ...
                # Filter sentence
                sentence_obj = self.__sentence_filter(sentence)
            ...
            if isinstance(element, LTFigure):
                ...
                if len(x0_list) > 0 and len(y1_list) > 0 and len(x1_list) > 0 and len(y0_list) > 0:
                    ...
                    # Filter sentence
                    sentence_obj = self.__sentence_filter(sentence_item)
                    if sentence_obj['is_valid_sentence'] is False:
                        index += 1
                        continue
                    sentence = sentence_obj['sentence']
    ...
    def __sentence_filter(self, sentence: str):
...
if __name__ == '__main__':
    pdf_block_extract = PdfBlockExtract('xxx.pdf', {'xxx': 'xxx'}, pages=None)
    sentence_list = pdf_block_extract.extract()
    sentence_list_json = []
    for item in sentence_list:
        print(item)
        sentence_list_json.append(item)
